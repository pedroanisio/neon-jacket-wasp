...
import normalize from "./normalize_v4.ts";
import type { V4LoadPayload } from "./shared/types.ts";

interface V4FileLoaderProps {
  readonly title?: string;
  readonly description?: string;
  readonly onLoad: (payload: V4LoadPayload) => void;
}

export default function V4FileLoader({
  title,
  description,
  onLoad,
}: V4FileLoaderProps): JSX.Element {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (!raw.contour || !Array.isArray(raw.contour)) {
            setError(
              "Invalid JSON: missing 'contour' array. Needs a v4 silhouette document.",
            );
            return;
          }
  ...
  return (
    ...
    >
      ...
      <div style={{ color: "#172030", fontSize: 9 }}>
        Expects v4 schema with: contour, landmarks, proportion, strokes, body_regions,
        biomechanics, style_deviation, shape_complexity, volumetric_estimates, convex_hull,
        width_profile, medial_axis
      </div>
      {error && (
        <div
          style={{
