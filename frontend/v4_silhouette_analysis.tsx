...
import { BODY_REGION_COLORS } from "./shared/colors.ts";
import { SCALE, CENTER_X, PAD_TOP, huToSvg } from "./shared/coord.ts";
...
export default function V4Renderer(): JSX.Element {
  ...
  if (!loaded) {
    return (
      ...
      >
        <V4FileLoader
          title="V4 ANALYSIS"
          description="Drop a silhouette v4 JSON for full analysis dashboard"
          onLoad={setLoaded}
        />
      </div>
    );
...
function V4RendererInner({ D, fileName, onReset }: V4InnerProps): JSX.Element {
  ...
  const contourPath = useMemo(() => {
    // BUG FIX: was referencing D.c — now correctly uses D.contour
    const r = D.contour.map(([dx, dy]) => huToSvg(dx, dy));
    const l = [...D.contour].reverse().map(([dx, dy]) => huToSvg(-dx, dy));
    return (
      "M " +
      [...r, ...l].map(([x, y]) => `${x.toFixed(1)},${y.toFixed(1)}`).join(" L ") +
      " Z"
    );
  }, [D.contour]);
  ...
  const lmData = useMemo(
    () =>
      // BUG FIX: was referencing D.l — now correctly uses D.landmarks
      D.landmarks.map((lm) => {
  ...
  // BUG FIX: was referencing D.l[3].dy — now correctly uses D.landmarks
  const neckDy = D.landmarks[3]?.dy ?? 1.26; // neck_valley = 1 HU
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 9, color: "#172030", marginTop: 2 }}>
          {/* BUG FIX: was referencing D.c.length and D.l.length */}
          {D.contour.length} contour pts · {D.strokes.length} strokes ·{" "}
          {D.landmarks.length} landmarks · {D.ar.length} body regions
      ...
      <svg viewBox="0 0 270 560" style={{ width: "100%", maxWidth: 420, height: "auto" }}>
          ...
          lmData.map((lm, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {lm.n.replace(/_/g, " ").slice(0, 12)}
                </text>
                {hov && (
                  <g>
                    <rect
                      x={100} y={lm.y - 16} width={150} height={12}
        ...
        {D.bio?.com_dy != null && (
          <g>
            <line
              x1={CX - 4} y1={PT + D.bio.com_dy * S}
              x2={CX + 4} y2={PT + D.bio.com_dy * S}
              stroke="#ef4444" strokeWidth={0.8} strokeDasharray="1,1"
            />
            <text
              x={CX + 6} y={PT + D.bio.com_dy * S + 2}
        ...
        {Array.from({ length: 7 }, (_, i) => {
          const y1 = PT + (D.landmarks[0]?.dy ?? 0) * S + i * neckDy * S;
      ...
      <div style={{ maxWidth: 720, width: "100%", padding: "12px 0" }}>
        {tab === "proportions" && (
          <div>
            <div style={{ fontSize: 10, color: "#3b5068", fontWeight: 600, marginBottom: 8, letterSpacing: 1 }}>
              SEGMENT RATIOS — {D.pr.hc.toFixed(1)} HEAD UNITS
            </div>
            {Object.entries(D.pr.sr).map(([k, v]) => (
              <Bar key={k} label={k.split("\u2192").map((s) => s.slice(0, 4)).join("\u2192")} value={v} max={0.3} color="#0ea5e9" />
            ))}
            <div style={{ fontSize: 10, color: "#3b5068", fontWeight: 600, marginTop: 14, marginBottom: 8, letterSpacing: 1 }}>
              COMPOSITE RATIOS
            </div>
            {Object.entries(D.pr.comp).map(([k, v]) => (
              <Stat key={k} label={k.replace(/_/g, " ")} value={typeof v === "number" ? v.toFixed(3) : String(v)} />
            ))}
            <div style={{ fontSize: 10, color: "#3b5068", fontWeight: 600, marginTop: 14, marginBottom: 6, letterSpacing: 1 }}>
              CANONICAL SYSTEMS
            </div>
            {D.pr.canons.map((c, i) => (
              <Stat key={i} label={c.sys} value={`${c.heads} heads`} accent="#f59e0b" />
            ))}
          </div>
        )}
          ...
          <div>
            ...
            {D.ar.map((r, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                <div style={{ width: 8, height: 8, borderRadius: 2, background: BODY_REGION_COLORS[i % BODY_REGION_COLORS.length], opacity: 0.7 }} />
                ...
                <div style={{ flex: 1, height: 6, background: "#0a1018", borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ width: `${r.f * 100 * 4.5}%`, height: "100%", background: BODY_REGION_COLORS[i % BODY_REGION_COLORS.length], opacity: 0.7, borderRadius: 2 }} />
                </div>
            ...
            {D.ar.map((r, i) => (
              <div key={i} style={{ display: "flex", gap: 12, fontSize: 8, color: "#3b5068", padding: "2px 0", borderBottom: "1px solid #0a1018" }}>
                <span style={{ width: 80, color: BODY_REGION_COLORS[i % BODY_REGION_COLORS.length] }}>{r.n}</span>
        ...
        {tab === "style" && D.sd && (
          <div>
            <div style={{ fontSize: 10, color: "#3b5068", fontWeight: 600, marginBottom: 4, letterSpacing: 1 }}>
              STYLE DEVIATION vs {D.sd.canon.replace(/_/g, " ").toUpperCase()}
            </div>
            <div style={{ fontSize: 8, color: "#1e3a5f", marginBottom: 10 }}>
              Figure: {D.sd.fh} heads · Canon: {D.sd.ch} heads · L² distance: {D.sd.l2}
            </div>
            ...
            {Object.entries(D.sc).map(([k, v]) => {
              ...
              return (
                <Stat
                  key={k}
                  label={k.replace(/_/g, " ")}
                  value={isObj ? (v as { value: number }).value : (v as number)}
                  unit={isObj ? ((v as { units?: string }).units ?? (v as { method?: string }).method ?? "") : ""}
                />
              );
            })}
        ...
        {tab === "bio" && D.bio && (
          <div>
            ...
            {D.bio.com_dy != null && <Stat label="Center of Mass (dy)" value={D.bio.com_dy.toFixed(3)} unit="HU" />}
            ...
            <svg viewBox="0 0 300 100" style={{ width: "100%", height: 80, background: "#0a1018", borderRadius: 4 }}>
              {D.wp.filter((w) => w.w > 0 && w.w < 3).map((w, i) => {
                ...
                const h = (w.w / 2.5) * 90;
                return <rect key={i} x={x} y={95 - h} width={3} height={h} fill="#0ea5e9" opacity={0.5} rx={1} />;
              })}
              <text x={5} y={10} fill="#1e3a5f" fontSize={6} fontFamily="inherit">Full Width (HU)</text>
              {[0, 2, 4, 6, 8].map((v) => (
                <text key={v} x={(v / 8) * 290 + 5} y={98} fill="#0d1520" fontSize={5} fontFamily="inherit">{v}</text>
              ))}
            </svg>
            ...
            <svg viewBox="0 0 300 60" style={{ width: "100%", height: 50, background: "#0a1018", borderRadius: 4 }}>
              <polyline fill="none" stroke="#f59e0b" strokeWidth={1} opacity={0.6} points={D.med.filter((m) => m.r > 0).map((m) => `${(m.dy / 8) * 290 + 5},${55 - (m.r / 1.3) * 45}`).join(" ")} />
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 12, marginTop: 16 }}>
        <div style={{ fontSize: 7, color: "#0d1520" }}>
          {fileName} · schema {D.version} · {D.contour.length} pts · {D.strokes.length} strokes · {D.landmarks.length} landmarks
        </div>
        <button
          onClick={onReset}
          style={{ background: "transparent", border: "1px solid #172030", color: "#3b5068", padding: "2px 8px", borderRadius: 3, fontSize: 9, cursor: "pointer", fontFamily: "inherit" }}
        >
          Load Another
        </button>
      </div>
